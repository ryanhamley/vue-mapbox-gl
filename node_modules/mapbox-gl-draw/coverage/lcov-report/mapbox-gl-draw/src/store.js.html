...
Store.prototype.delete = function(featureIds, options = {}) {
  toDenseArray(featureIds).forEach(id =&gt; {
    ...
    if (!options.silent) {
      if (this._deletedFeaturesToEmit.indexOf(this._features[id]) === -1) <span class="missing-if-branch" title="else path not taken" >E</span>{
...
Store.prototype.deselect = function(featureIds, options = {}) {
  toDenseArray(featureIds).forEach(id =&gt; {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!this._selectedFeatureIds.has(id)) <span class="cstat-no" title="statement not covered" >return;</span>
    this._selectedFeatureIds.delete(id);
    this._changedFeatureIds.add(id);
    if (!options.silent) {
      this._emitSelectionChange = true;
